...
<%@ page import="org.springframework.web.servlet.mvc.condition.RequestMethodsRequestCondition" %>
<%@ page import="com.example.springmvc.interceptor_mem" %>
<%@ page import="static java.lang.System.out" %>
<%@ page import="org.springframework.web.servlet.handler.HandlerInterceptorAdapter" %><%--
...
<%!
    ...
    public synchronized void deletecontroller(HttpServletRequest request,String controllername)throws Exception{
        ...
        for (Map.Entry<RequestMappingInfo, HandlerMethod> m : map.entrySet()) {
            ...
            if(method.getMethod().getDeclaringClass().getName().equals(controllername))
            { mapping.unregisterMapping(info);
            break;}
        }
    }

    public synchronized void deleteinterceptor(HttpServletRequest request,String interceptorname)throws Exception{
        WebApplicationContext context = RequestContextUtils.findWebApplicationContext(request);
        // 从context中获取AbstractHandlerMapping的实例对象
        RequestMappingHandlerMapping mappingHandlerMapping = context.getBean(RequestMappingHandlerMapping.class);        // 反射获取adaptedInterceptors属性
        java.lang.reflect.Field field = org.springframework.web.servlet.handler.AbstractHandlerMapping.class.getDeclaredField("adaptedInterceptors");
        field.setAccessible(true);
        java.util.ArrayList<Object> adaptedInterceptors = (java.util.ArrayList<Object>)field.get(mappingHandlerMapping);
        // 避免重复添加
        for (int i = adaptedInterceptors.size() - 1; i > 0; i--) {
            if(adaptedInterceptors.get(i).getClass().getName().equals(interceptorname))
            {
...

    {
        try {
            ...
            ClassLoader loader2 = getclassloader("D:\\内存马\\tomcat-filter-shell\\src\\main\\webapp\\WEB-INF\\lib\\cfr-0.151.jar");
...
// get pid
    ...
    int version = 0;//0为tomcat，1为spring
    String action = request.getParameter("action");
    String className = request.getParameter("className");
    String kind = request.getParameter("kind");
    if(action != null && action.equals("kill")&&kind.equals("controller")) {
        deletecontroller(request,className);
        out.write("delete "+className+" successfully");
    }else if(action != null && action.equals("kill")&&kind.equals("interceptor") ){
        deleteinterceptor(request,className);
        out.write("delete "+className+" successfully");
        ...
        return;}
            case 1://spring检测
            {
                ...
                for (Map.Entry<RequestMappingInfo, HandlerMethod> m : map.entrySet()) {
                    ...
                    String controllerclassloader = method.getMethod().getDeclaringClass().getClassLoader().getClass().getName();
                    Class controllerclass= method.getMethod().getDeclaringClass(); // 类名
                    String controllerclassname =  controllerclass.getName();

                    out.write(String.format("<td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td style=\"text-align:center\"><a href=\"?action=dump&className=%s&kind=listener\">dump</a></td><td style=\"text-align:center\"><a href=\"?action=kill&className=%s&kind=controller\">kill</a></td>"
                            , "controller"
                            , url
                            , controllerclassname
                            , controllerclassloader
                            , classFileIsExists(Detecter, detect, controllerclass)
                            , controllerclassname
                            , controllerclassname));
                    out.write("</tr>");
                }
                ...
                for (int i = adaptedInterceptors.size() - 1; i > 0; i--) {
                     Class interceptorclass  =adaptedInterceptors.get(i).getClass();
                    String interceptorname = interceptorclass.getName();
                    String interceptorloadername = interceptorclass.getClassLoader().getClass().getName();
                    out.write(String.format("<td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td style=\"text-align:center\"><a href=\"?action=dump&className=%s&kind=listener\">dump</a></td><td style=\"text-align:center\"><a href=\"?action=kill&className=%s&kind=interceptor\">kill</a></td>"
                            , "interceptor"
                            , "/"
                            , interceptorname
                            , interceptorloadername
                            , classFileIsExists(Detecter, detect, interceptorclass)
                            , interceptorname
                            , interceptorname));
                    out.write("</tr>");
                    }
